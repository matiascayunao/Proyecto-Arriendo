@extends('template.master', ['tituloPagina' => 'Editar Vehiculo'])

@section('contenido')

<div class="col">
    <div class="card">
        <div class="card-header bg-dark text-white">Edite los datos del Vehiculo</div>
        <div class="card-body">
            {{-- mensajes de error --}}
            @if($errors->any())
            ...
            @endif
            {{-- /mensajes de error --}}
            <form method="POST" action="{{ route('vehiculos.update', $vehiculo->matricula) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row">
                    ...
                    <div class="mb-3 col-12 col-md-6">
                        ...
                        <input type="text" id="nombre_vehiculo" name="nombre_vehiculo" class="form-control" value="{{ $vehiculo->nombre_vehiculo }}">
                    ...
                    <div class="mb-3">
                        <label class="form-label" for="tipo_id">Tipo de Vehiculo</label>
                        <select id="tipo_id" name="tipo_id" class="form-control @error('tipo_id') is-invalid @enderror">
                            <option value="0">Seleccione</option>
                            @foreach($tipos as $tipo)
                            <option value="{{$tipo->id}}" @if(old('tipo_id')==$tipo->id) selected @endif>{{ $tipo->tipo_vehiculo }}</option>
                            @endforeach
                        </select>
                        @error('tipo_v')
